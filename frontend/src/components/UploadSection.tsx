...
const UploadSection: React.FC<UploadSectionProps> = ({ onFileUpload }) => {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      // Clone the file to prevent changes
      const clonedFile = new File([file], file.name, { type: file.type, lastModified: file.lastModified });

      setIsUploading(true);
      try {
        onFileUpload(clonedFile);
      } catch (err) {
    ...
    <div className="max-w-3xl mx-auto py-12 px-4">
      ...
      <div
        className={`
          ...
          ${isDragOver ? 'border-indigo-500 bg-indigo-50/20 scale-105 shadow-2xl' : 'border-slate-200 bg-white/10 hover:border-indigo-400 hover:bg-indigo-50/10'}
      ...
      >
        ...
        <div className="space-y-8">
          ...
          <div>
            <h3 className="text-2xl font-bold text-slate-800 mb-3 animate-fade-in">
              {isUploading ? 'Uploading your resume...' : isDragOver ? 'Drop your resume here' : 'Drag & drop your resume'}
            </h3>
            <p className="text-slate-500 mb-4 text-base">
              {!isUploading ? (
                <>
                  or{' '}
                  <label htmlFor="resume-upload" className="text-indigo-600 hover:text-indigo-800 cursor-pointer font-semibold transition-colors duration-200">
                    browse files
                  </label>
                </>
              ) : (
        ...
        {isDragOver && !isUploading && (
          <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 to-blue-500/20 rounded-3xl flex items-center justify-center animate-pulse">
            <div className="bg-indigo-600 text-white px-6 py-3 rounded-full font-semibold shadow-lg">Drop to upload</div>
          </div>
        )}
      </div>
